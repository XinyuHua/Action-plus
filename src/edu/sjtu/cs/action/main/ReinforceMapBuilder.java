 ...
 */
public class ReinforceMapBuilder {
    ...
    private HashMap<String, List<String>> curMap;
    private HashMap<String, List<Double>> curScoreMap;
    ...
    public ReinforceMapBuilder(int nounIdx, int acIdx, int stride){
...
/*
    public void buildReinforced(int lower, int upper)throws Exception{
        loadNounInTitle(lower, upper);
        int size = upper - lower;
        if(size % stride != 0){
            System.out.println("Please choose a suitable stride");
            return;
        }

        int times = size / stride;
        curMap = new HashMap<>();
        curScoreMap = new HashMap<>();
        for(int i = 0; i < times; ++i){
            loadAcInBody(lower + i * stride, lower + i * stride + stride);
            computeFreq();
            computeTfidf();
        }
        writeToDisk();
    ...
    private void loadAcInBody(int lower, int upper)throws Exception{
        documentSize = 0;
        String line;
        HashMap<String, List<String>> nounsInAction = new HashMap<>();
        for(int i = lower; i < upper; ++i){
            ...
            while((line = acDatReader.readLine())!=null){
                ...
                for(String action : tmpAcSet){
                    ...
                    if(curMap.containsKey(action)){
                        //List<String> nounList =
                        List<String> sortedNounForAction = curMap.get(action);
                        for(int k = 0; k < sortedNounForAction.size() / 2; ++k){
                            String noun = sortedNounForAction.get( k );
                            if(tmpNounArry.contains(noun)){
...
/*
    private void computeTfidf()throws Exception{
